    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            for i in range(boxes.shape[0]):
                ...
                #  print("detecting face takes: {}s".format(end - start))
                aligned_face = cv2.resize(aligned_face, (112,112))

                #  aligned_face = aligned_face - 127.5
                #  aligned_face = aligned_face * 0.0078125
                aligned_face = model.get_input(aligned_face)
            ...
            if len(faces)>0 :
                ...
                for face in faces: 
                    ...
                    added = model.get_feature(np.array(face))
                    #  cv2.imwrite("hello{}.jpg".format(frame), added)
                    added = np.array([added])
                    embeds = np.concatenate((embeds, added), 0)


                end = time.time()
                #  print("Extracting features takes: {}s".format(end-start))
                ...
                for embedding in embeds:
                    ...
                    #  min_20_dist = np.argsort(dist)
                    #  min_dist = min_20_dist[0]
                    #  args = min_20_dist[:10]
                    #  name = operator.itemgetter(*args)(names)
                    #  name = mode(name)[0][0]
                    if dist[min_dist] < threshold:
                        #  predictions.append(name)
                        predictions.append(names[min_dist])
                        #  print(dist[idx])
                    else:
